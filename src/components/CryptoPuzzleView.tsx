...
export const CryptoPuzzleView: React.FC<CryptoPuzzleViewProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const extraChars = Array.from({ length: extraCount }).map(() => chars.charAt(Math.floor(Math.random() * chars.length)));
    
    const combined = [...answerChars, ...extraChars].sort(() => Math.random() - 0.5);
  ...
  if (gameComplete) {
    return (
      <div className="w-full h-full bg-on-background text-background flex flex-col items-center justify-center font-sans">
        ...
        <h1 className="text-4xl sm:text-5xl font-serif italic mb-4">اكتمل التحدي</h1>
        <p className="text-lg opacity-80 mb-8 max-w-lg text-center leading-relaxed">
          أحسنت! لقد تمكنت من حل جميع الألغاز البرمجية والتشفيرية بنجاح. معرفتك العملية ممتازة.
        </p>
        <button 
          onClick={onBack}
  ...
  return (
    <div className="w-full h-full bg-surface-variant flex flex-col font-sans text-on-background relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#1A1A1A 2px, transparent 2px)', backgroundSize: '40px 40px' }}></div>
      
      <header className="flex justify-between items-end p-8 border-b border-on-background/10 relative z-10 shrink-0">
        <div>
          <button onClick={onBack} className="flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold opacity-50 hover:opacity-100 hover:text-primary mb-4 transition-colors">
            <LogOut className="w-3 h-3" /> العودة للمركز
          </button>
          <h1 className="text-4xl sm:text-5xl font-serif italic font-light tracking-tighter leading-none">تشفير وأوامر</h1>
          <p className="text-[10px] uppercase tracking-[0.2em] mt-2 opacity-60 font-semibold">حل الشفرات التشغيلية</p>
        </div>
        <div className="hidden sm:block text-right">
           <p className="text-[10px] uppercase tracking-widest opacity-50">التقدم</p>
           <p className="font-serif italic font-bold text-2xl">{currentIndex + 1} / {currentPuzzles.length}</p>
        </div>
      </header>

      <main className="flex-1 flex flex-col items-center p-4 sm:p-8 relative z-10 overflow-y-auto">
        <div className="w-full max-w-3xl flex flex-col mt-4 sm:mt-12">
          ...
          <div className="mb-12">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-serif leading-relaxed font-light text-right" dir="auto">
               "{currentPuzzle.question}"
            </h2>
          </div>

          {/* Slots area */}
          <div className={`flex flex-wrap gap-2 lg:gap-3 mb-16 justify-center ${isError ? 'animate-shake' : ''}`} dir="ltr">
            {slots.map((poolId, i) => (
              <div 
                key={i}
          ...
          <div className="flex justify-center mb-8 gap-4">
             ...
             >
               ...
               <span className="text-[10px] uppercase tracking-widest font-bold">مسح الإجابة</span>
             </button>
          </div>

          {/* Available letters pool */}
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 max-w-2xl mx-auto" dir="ltr">
             {pool.map((p) => (
               <button
                 key={p.id}
                 ...
                 className={`w-10 h-12 sm:w-14 sm:h-16 font-mono text-xl sm:text-2xl font-bold flex items-center justify-center transition-all ${
                   p.used 
                     ...
                     : 'bg-on-background text-background hover:bg-primary hover:-translate-y-1 shadow-[2px_2px_0px_#B44C32] sm:shadow-[4px_4px_0px_#B44C32]'
                 }`}
               >
                 {p.char}
               </button>
          ...
          <div className="flex flex-col items-center mt-12 gap-4 border-t border-on-background/10 pt-8 w-full max-w-md mx-auto">
            <p className="text-[10px] uppercase tracking-widest font-bold opacity-50">تلميح الإدارة</p>
            <input 
              type="password"
