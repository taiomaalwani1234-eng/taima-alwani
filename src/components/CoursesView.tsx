...
const MOCK_COURSES: Course[] = [
  {
    ...
    lectures: [
      ...
      },
      {
        ...
        contentType: "pdf",
        contentUrl:
          "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf", // Example dummy pdf
        duration: "10 صفحات",
      },
    ],
  },
  {
    ...
    lectures: [
      {
        id: "c2-l1",
        title: "فيديوهات لتعليم أمن المعلومات",
        desc: "مجموعة من الفيديوهات التعليمية التي تغطي أساسيات ومفاهيم أمن المعلومات المختلفة.",
        contentType: "video",
        contentUrl: "https://www.youtube.com/embed/dQw4w9WgXcQ",
        duration: "22:10",
      },
...
export const CoursesView: React.FC<CoursesViewProps> = ({
  ...
  const inactiveLectureClass =
    "border-outline/20 hover:border-primary hover:text-primary";
  ...
  return (
    <div
      className={`w-full h-full flex flex-col font-sans transition-colors duration-500 overflow-hidden relative ${bgClass}`}
      dir="rtl"
    >
      {showTutorial && (
        <TutorialOverlay
          message="استكشف الدورات والمكتبات التعليمية لتطوير مهاراتك بشكل مستمر. الدخول عبر بطاقات واختيار المقاطع لتكثيف المعرفة."
          onDismiss={() => setShowTutorial(false)}
        />
      )}
      {/* Header */}
      <header
        className={`p-4 border-b ${borderClass} flex flex-col sm:flex-row justify-between items-start sm:items-center shrink-0`}
      >
        <div className="flex items-center gap-4 mb-4 sm:mb-0">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-[0.2em] mt-1 opacity-60 font-semibold">
              {selectedCourse
                ? "بيئة التعلم المدمج (LMS)"
                : "مسارات التعلم الأكاديمي الشامل"}
            </p>
          </div>
        </div>
        <button
      ...
      {!selectedCourse ? (
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-8">
          <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {MOCK_COURSES.map((course) => (
              ...
              >
                <div
                  className={`w-14 h-14 mb-6 rounded-none flex items-center justify-center bg-primary/10 ${primaryText} group-hover:scale-110 transition-transform`}
                >
                  <BookOpen className="w-7 h-7" />
                </div>
                <h3 className="text-xl sm:text-2xl font-serif font-bold mb-4 line-clamp-2 leading-snug">
                  {course.title}
                </h3>
                <p className="text-sm opacity-70 mb-8 flex-1 leading-relaxed font-medium">
                  {course.desc}
                </p>
                <div
                  className={`w-full py-3 inline-flex justify-center text-[10px] uppercase tracking-widest font-bold border ${borderClass} bg-transparent group-hover:bg-primary group-hover:text-on-primary group-hover:border-primary transition-colors`}
                >
                  دخول للكورس
                </div>
              </button>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <aside
            className={`w-full md:w-80 border-l ${borderClass} ${sidebarClass} flex flex-col overflow-y-auto shrink-0 transition-colors duration-500`}
          >
          ...
          {/* Lecture Content */}
          <main
            className={`flex-1 overflow-y-auto transition-colors duration-500 ${contentBgClass} relative`}
          >
            {selectedLecture ? (
              <div className="max-w-4xl mx-auto p-4 sm:p-8 flex flex-col h-full">
                <div className="mb-6">
                  <h2 className="text-2xl sm:text-4xl font-serif font-light mb-2">
                ...
                <div
                  className={`flex-1 rounded-sm border ${borderClass} overflow-hidden bg-black/5`}
                >
                  {selectedLecture.contentType === "video" &&
                    selectedLecture.contentUrl && (
                  ...
                  {selectedLecture.contentType === "pdf" &&
                    selectedLecture.contentUrl && (
                      <div className="w-full h-full min-h-[500px] flex flex-col items-center justify-center p-8 text-center text-primary relative">
                        <div className="absolute top-4 right-4">
                          <button
                            onClick={() => setSelectedLectureId(null)}
                            className={`px-4 py-2 border border-primary/30 hover:bg-primary/10 transition-colors text-[10px] uppercase tracking-widest font-bold font-sans inline-flex items-center gap-2 rounded`}
                          >
                            <ChevronRight className="w-4 h-4" /> العودة
                          </button>
                        </div>
                        <FileText className="w-16 h-16 mb-4 opacity-50" />
                        <p className="mb-4 text-sm font-bold text-inherit">
                          عارض الـ PDF يتطلب مكتبة إضافية أو إطار متصفح لدعمه في
                          هذه البيئة.
                        </p>
                        <a
                  ...
                  {selectedLecture.contentType === "markdown" &&
              ...
              <div className="w-full h-full flex items-center justify-center p-8 text-center">
                <div>
                  ...
                  <p className="opacity-70 text-sm">
                    اختر محاضرة من القائمة الجانبية للبدء.
                  </p>
                </div>
              </div>
            )}
